...
public class TentacleMovement : MonoBehaviour
{
    ...
    private void TryChangeTentacleAnchor()
    {
        if(Vector3.Distance(_targetLocation, transform.position) > 0.5f)
        {
            ...
            {
                if (RaycastTentacle(_tentacles[_currentAnchorChangeIndex]))
                {
    ...
    private bool RaycastTentacle(TentacleAnchor anchor)
    {
        if(Vector3.Distance(_targetLocation, transform.position) > 0.5f)
        {
            ...
            anchor.LaunchTentacle(_tentacleRaycastHit[0].point, _tentacleLaunchSpeed);
            return true;
        }
        return false;
    }
